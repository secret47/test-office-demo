    ...
    <el-menu class="menu" :collapse="isCollapsed" :style="{ width: sidebarWidth + 'px' }" :unique-opened="true">
        ...
        <el-sub-menu :index="item.name" v-for="(item, index) in filteredRoutes" :key="item.name">
            ...
            <el-menu-item :index="subItem.name" v-for="(subItem, subIndex) in item.children" :key="subItem.name"
                ...
                <template #title>
                    <span>
                        <el-icon v-if="subItem.meta.icon" :size="20">
                ...
                <el-sub-menu v-if="subItem.children && subItem.children.length > 0" :index="subItem.name + '_sub'"
                    :key="subItem.name + '_sub'" @click="handleMenuItemClick(subItem)">
                    <template #title>
                        <span>
                            <el-icon v-if="subItem.meta.icon" :size="20">
                                <component :is="subItem.meta.icon"></component>
                            </el-icon>
                            <span>{{ subItem.meta.title + '(展开)' }}</span>
                        </span>
                    </template>
                    <el-menu-item :index="subSubItem.name" v-for="(subSubItem, subSubIndex) in subItem.children"
                        :key="subSubItem.name" @click="handleMenuItemClick(subSubItem)">
                        <template #title>
                            <span>
                                <el-icon v-if="subSubItem.meta.icon" :size="20">
                                    <component :is="subSubItem.meta.icon"></component>
                                </el-icon>
